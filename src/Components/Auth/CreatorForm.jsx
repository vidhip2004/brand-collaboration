...
const CreatorForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess("Creator account created successfully! Redirecting to dashboard...");
      setTimeout(() => {
  ...
  return (
    <form className="space-y-4" onSubmit={handleSubmit}>
      ...
      <div className="flex items-start gap-2.5 pt-1">
        <input
          type="checkbox"
          id="creatorAgreeToTerms"
          name="agreeToTerms"
          checked={formData.agreeToTerms}
          onChange={handleChange}
          className="mt-1 accent-[#8B6F5A] cursor-pointer rounded"
        />
        <label htmlFor="creatorAgreeToTerms" className="text-[#4A3A2E]/80 text-xs font-medium cursor-pointer">
